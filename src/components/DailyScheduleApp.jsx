  ...
  const attendanceStats = useMemo(() => {
    ...
    data.forEach(row => {
      ...
      studentNames.forEach((name, idx) => {
        ...
        } else if (studentStatus.includes("취소") || studentStatus.includes("중단")) {
          canceled += headcount;
          ...
          {attendanceStats && (
            <div className="mb-8 grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <div className="bg-red-50 rounded-2xl p-4 border border-red-100 flex flex-col items-center justify-center shadow-sm">
                <span className="text-red-600 text-xs font-black mb-1 uppercase tracking-wider">결석/취소</span>
                    ...
                    <tbody className="text-gray-700 text-xs md:text-sm">
                      {scheduleData.length === 0 ? (
                        ...
                        scheduleData.map((row, index) => {
                          ...
                          if (row.status) {
                            const hasAbsenceOrCancel = row.status.includes("결석") || row.status.includes("취소");

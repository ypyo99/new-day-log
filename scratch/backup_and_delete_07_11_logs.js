...
async function fetchAllData() {
  ...
  console.log("1. 데이터 백업을 위해 Supabase에서 7월~11월 데이터 다운로드 중...");

  while (hasMore) {
    const url = `${SUPABASE_URL}/rest/v1/daily_logs?log_date=gte.2026-07-01&log_date=lte.2026-11-30&order=id.asc&limit=${limit}&offset=${offset}`;
...
async function verifyCount() {
  const url = `${SUPABASE_URL}/rest/v1/daily_logs?log_date=gte.2026-07-01&log_date=lte.2026-11-30`;
  const res = await fetch(url, {
    method: 'HEAD',
    headers: {
      'apikey': SUPABASE_KEY,
...
async function run() {
  try {
    ...
    console.log(`삭제 대상 전체 7월~11월 데이터 수: ${expectedCount}개`);

    if (expectedCount === 0) {
      console.log("삭제할 7월~11월 데이터가 없습니다.");
      return;
    }
    ...
    // 4. 로컬 JSON 백업 파일 저장
    const backupFilePath = path.join(__dirname, 'backup_daily_logs_2026_07_11.json');
    fs.writeFileSync(backupFilePath, JSON.stringify(backupData, null, 2), 'utf-8');
    console.log(`백업 파일 저장 완료: ${backupFilePath}`);

    // 5. 데이터 삭제 실행
    console.log("2. Supabase에서 7월~11월 데이터 삭제 실행 중...");
    const deleteUrl = `${SUPABASE_URL}/rest/v1/daily_logs?log_date=gte.2026-07-01&log_date=lte.2026-11-30`;
    ...
    console.log(`삭제 후 남은 7월~11월 데이터 수: ${finalCount}개`);
